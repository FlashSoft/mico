...
# 配置nodered的接收地址
nodered_url="http://10.0.1.70:1880/endpoint/miai"
...
# NodeRed账号密码
nodered_auth="sheephome:expo2010!"
# 小爱asr日志地址
...
while true;do
  ...
  if [[ ${new_md5} != ${res_md5} ]];then
    ...
    #curl --insecure –connect-timeout 2 -m 2 -s -u "${nodered_auth}" --data-urlencode "asr=${asr_content}" --data-urlencode "res=${res_content}" "${nodered_url}/set/log"
