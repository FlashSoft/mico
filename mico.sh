...
echo ""
header=`curl –connect-timeout 2 -m 2 -sI -u "${nodered_auth}" ${nodered_url}`
...
while true;do
  ...
  if [[ ${new_md5} != ${res_md5} ]];then
    ...
    log_res=`curl –connect-timeout 2 -m 4 -s -u "${nodered_auth}" --data-urlencode "asr=${asr_content}" --data-urlencode "res=${res_content}" "${nodered_url}/miai/set/log"`
    echo "== 投日志 | ${log_res}"
    
    if ([[ ! -z ${res_keywords} ]] && [[  ! -z `echo "${res_content}"|awk 'match($0,/'${res_keywords}'/){print 1}'` ]]) || [ ${miai_errcode} ];then
      echo "== 试图停止"
  ...
  if [[ "${keywords_update_timeout}" -gt "0" ]];then
    ...
    if [[ "$step" -gt "${keywords_update_timeout}" ]];then
        res_keywords=`curl --insecure -u "${nodered_auth}" –connect-timeout 2 -m 4 -s ${nodered_url}`
        echo "== 更新关键词 | 关键词内容: ${res_keywords}"
        last_time=`date +%s`
    fi
  fi
  # usleep 10
